...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var standOut = readData("standOut");
        var layout = readData("layout");
        var presentationView = readData("presentationView");
        var attractive = readData("attractive");
        var enjoy = readData("enjoy");
        var suggestions = readData("suggestions");
        res.render('showResults', {results: [standOut, layout, presentationView, attractive, enjoy, suggestions]});
        console.log([standOut, layout, presentationView, attractive, enjoy, suggestions]);
    });
    ...
    app.post('/survey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "attractive") && (json[key].length > 1) && json[key] !== "Color"
            && json[key] !== "Icons" && json[key] !== "Pictures" && json[key] !== "Organization"){
